...
-module(antidote_crdt_secure_counter_b).
...
-export_type([ antidote_crdt_secure_counter_b/0,
               antidote_crdt_secure_counter_b_op/0,
               id/0
            ]).

-type antidote_crdt_secure_counter_b() :: {orddict:orddict(), orddict:orddict(), integer()}.

-type antidote_crdt_secure_counter_b_op() :: antidote_crdt_secure_counter_b_anon_op() | antidote_crdt_secure_counter_b_src_op().

-type antidote_crdt_secure_counter_b_anon_op() :: {transfer, {pos_integer(), id(), id()}}
                                                | {increment, {pos_integer(), pos_integer(), id()}}
                                                | {increment, {pos_integer(), id()}}
                                                | {decrement, {pos_integer(), pos_integer(), id()}}
                                                | {decrement, {pos_integer(), id()}}.

-type antidote_crdt_secure_counter_b_src_op() :: {antidote_crdt_secure_counter_b_anon_op(), id()}.

-type id() :: term(). %% A replica's identifier.

%% @doc Return a new, empty secure bound counter.
-spec new() -> antidote_crdt_secure_counter_b().
new() ->
...
-spec localPermissions(id(), antidote_crdt_secure_counter_b()) -> non_neg_integer().
...
-spec permissions(antidote_crdt_secure_counter_b()) -> {non_neg_integer() | nil, non_neg_integer() | nil}.
...
%% @doc Return the read value of a given `antidote_crdt_secure_counter_b()', itself.
...
%% and the third parameter is a `antidote_crdt_secure_counter_b()' which holds the current snapshot.
%%
...
-spec downstream(antidote_crdt_secure_counter_b_op(), antidote_crdt_secure_counter_b()) -> {ok, term()} | {error, no_permissions}.
...
-spec update(term(), antidote_crdt_secure_counter_b()) -> {ok, antidote_crdt_secure_counter_b()}.
...
%% doc Return the binary representation of a `antidote_crdt_secure_counter_b()'.
-spec to_binary(antidote_crdt_secure_counter_b()) -> binary().
to_binary(SecureBCounter) ->
    term_to_binary(SecureBCounter).

%% doc Return a `antidote_crdt_secure_counter_b()' from its binary representation.
